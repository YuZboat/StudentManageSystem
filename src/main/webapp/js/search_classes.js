...
function showData(page) {
	var url = "";
	url += APP_CONTEXT + "/SearchClassesServlet?search_type="
			+ $("#search_type").val();
	...
	$.post(url, null, function(rs) {
		...
		for ( var i = 0; i < rs.length; i++) {
			...
			str += "<td><a href='" + APP_CONTEXT + "/EditClassesServlet?cla_id="
					+ rs[i][0].id + "'>编辑</a> / ";
			str += "<a href='" + APP_CONTEXT + "/DeleteClassesServlet?cla_id="
					+ rs[i][0].id + "'>删除</a></td>";
			str += "</tr>";
			$("#table").append(str);
		}
...
function getCountPage() {
	var url = "", rt = "";
	url += APP_CONTEXT + "/GetClassesCountPageServlet?search_type="
			+ $("#search_type").val();
