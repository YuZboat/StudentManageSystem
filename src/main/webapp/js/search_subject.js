...
{
	var url = "";
	url += APP_CONTEXT + "/SearchSubjectServlet?search_type=" + $("#search_type").val();
	...
	$.post(url, null, function(rs)
	{
		...
		for ( var i = 0; i < rs.length; i++)
		{
			...
			if (roleId == 1)
			{
				str += "<td><a href='" + APP_CONTEXT + "/EditSubjectServlet?sub_id=" + rs[i].id + "'>编辑</a> / ";
				str += "<a href='" + APP_CONTEXT + "/DeleteSubjectServlet?sub_id=" + rs[i].id + "'>删除</a></td>";
			}
			else
				str += "<td> &nbsp;/</td>";
			str += "</tr>";
...
{
	var url = "", rt = "";
	url += APP_CONTEXT + "/GetSubjectCountPageServlet?search_type=" + $("#search_type").val();
...
{
	var rol_id;
	var url = APP_CONTEXT + "/LoginServlet?type=get_rol_id";
	$.ajax(
	{
		type : "post",
		url : url,
