...
function showData(page) {
	var url = "";
	url += APP_CONTEXT + "/SearcCla2SubServlet?search_type="
			+ $("#search_type").val();
	...
	$.post(url, null, function(rs) {
		...
		for ( var i = 0; i < rs.length; i++) {
			...
			str += "</tr>";
			var stra = "<a href='" + APP_CONTEXT + "/ExportCal_subServlet?search_type="
...
function getCountPage() {
	var url = "", rt = "";
	url += APP_CONTEXT + "/GetCla2SubCountPageServlet?search_type="
			+ $("#search_type").val();
