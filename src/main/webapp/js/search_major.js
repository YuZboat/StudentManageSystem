...
function showData(page) {
	var url = "";
	url += APP_CONTEXT + "/SearchMajorServlet?search_type=" + $("#search_type").val();
	...
	$.post(url, null, function(rs) {
		...
		for ( var i = 0; i < rs.length; i++) {
			...
			str += "<td><a href='" + APP_CONTEXT + "/EditMajorServlet?maj_id=" + rs[i].id
					+ "'>编辑</a> / ";
			str += "<a href='" + APP_CONTEXT + "/DeleteMajorServlet?maj_id=" + rs[i].id
					+ "'>删除</a></td>";
			str += "</tr>";
			$("#table").append(str);
		}
...
function getCountPage() {
	var url = "", rt = "";
	url += APP_CONTEXT + "/GetMajorCountPageServlet?search_type="
			+ $("#search_type").val();
