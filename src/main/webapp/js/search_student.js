...
function showData(page) {
	var url = "";
	url += APP_CONTEXT + "/SearchStudentServlet?search_type="
			+ $("#search_type").val();
	...
	$.post(url, null, function(rs) {
		...
		for ( var i = 0; i < rs.length; i++) {
			...
			if (roleId == 1) {
				str += "<td><a href='" + APP_CONTEXT + "/EditStudentServlet?stu_id="
						+ rs[i].id + "'>编辑</a> / ";
				str += "<a href='" + APP_CONTEXT + "/DeleteStudentServlet?stu_id="
						+ rs[i].id + "'>删除</a></td>";
			} else
				str += "<td>&nbsp;/</td>";
			str += "</tr>";
...
function getCountPage() {
	var url = "", rt = "";
	url += APP_CONTEXT + "/GetStudentCountPageServlet?search_type="
			+ $("#search_type").val();
