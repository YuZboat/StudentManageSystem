...
function showData(page) {
	var url = "";
	url += APP_CONTEXT + "/SearchScoreServlet?search_type=" + $("#search_type").val();
	...
	$.post(url, null, function(rs) {
		...
		for ( var i = 0; i < rs.length; i++) {
			...
			if (roleId == 2)
				str += "<td><a href='" + APP_CONTEXT + "/EditScoreServlet?sco_id="
						+ rs[i].id + "'>编辑</a></td>";
			else
				str += "<td>&nbsp;/ </td>";
			str += "</tr>";
...
function getCountPage() {
	var url = "", rt = "";
	url += APP_CONTEXT + "/GetScoreCountPageServlet?search_type="
			+ $("#search_type").val();
